# Graph Report - saude-orquestrator-llm  (2026-08-14)

## Corpus Check
- 22 files · ~4,564 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 120 nodes · 159 edges · 24 communities (14 shown, 10 thin omitted)
...
4. `sanitize_for_provider()` - 10 edges
5. `SemanticRouter` - 8 edges
6. `LLMRouter` - 7 edges
7. `StripAnthropicBeta` - 6 edges
8. `extract_prompt_text()` - 5 edges
9. `sanitize_messages_preserve_system()` - 5 edges
10. `normalize_tool_call_ids()` - 4 edges
...
Cohesion: 0.19
Nodes (6): Desvia de Mistral quando a requisição envolve tool calling. Confirmado via…, SmartRouterV2, has_tool_history(), Bug C (agravante): LLMLingua pode corromper metadados de tool_calls., should_skip_compression(), CustomLogger
...
- **Why does `SmartRouterV2` connect `SmartRouterV2` to `smart_router.py`, `RouterCache`, `TokenReducer`?**
