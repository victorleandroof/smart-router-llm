# Graph Report - saude-orquestrator-llm  (2026-08-15)

## Corpus Check
- 23 files · ~5,446 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 136 nodes · 183 edges · 25 communities (15 shown, 10 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 13 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `972a740c`
...
7. `LLMRouter` - 7 edges
8. `QwenToolCallingLLM` - 5 edges
9. `extract_prompt_text()` - 5 edges
10. `sanitize_messages_preserve_system()` - 5 edges
...
Cohesion: 0.11
Nodes (11): LLMRouter, SemanticRouter, Desvia de Mistral quando a requisição envolve tool calling. Confirmado via…, SmartRouterV2, has_tool_history(), Bug C (agravante): LLMLingua pode corromper metadados de tool_calls., should_skip_compression(), CustomLogger (+3 more)
...
Cohesion: 0.21
Nodes (14): fix_message_ordering(), _flatten_content(), _generate_valid_id(), _is_mistral_model(), _is_ollama_model(), normalize_assistant_content(), normalize_content_for_ollama(), normalize_tool_call_ids() (+6 more)
...
Cohesion: 0.32
Nodes (6): _extract_message_text(), extract_prompt_text(), Extrai o texto relevante para classificação de tier. Usa apenas a última…, Preserva o role 'system' original. Não converte system → user (diferente do…, sanitize_messages_preserve_system(), _strip_problematic_headers()

### Community 24 - "qwen_custom_llm.py"
Cohesion: 0.31
Nodes (7): _build_ollama_request(), _call_ollama_chat(), _extract_tool_use(), QwenToolCallingLLM, Monta o payload não-streaming para /api/chat, espelhando o formato que…, Contorna o bug de streaming do provider `ollama_chat` (LiteLLM 1.61.20) que…, CustomLLM
...
- **Why does `RouterCache` connect `RouterCache` to `smart_router.py`?**
  _High betweenness centrality (0.104) - this node is a cross-community bridge._
- **Why does `TokenReducer` connect `TokenReducer` to `smart_router.py`?**
